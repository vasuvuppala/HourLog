...
(
	...
	original_event INTEGER UNSIGNED COMMENT 'event is obsolete if this is not null',
	note INTEGER UNSIGNED,
	version INTEGER NOT NULL DEFAULT 0,
...
) ENGINE=InnoDB COMMENT='Each row is an event occurring in a facilty';
...
ALTER TABLE breakdown_event ADD CONSTRAINT FK_breakdown_event_breakdown_status 
	FOREIGN KEY (status) REFERENCES breakdown_status (brk_status_id)
	ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE event ADD CONSTRAINT FK_event_note 
	FOREIGN KEY (note) REFERENCES note (id)
	ON DELETE RESTRICT ON UPDATE CASCADE;

ALTER TABLE event ADD CONSTRAINT FK_event_event 
	FOREIGN KEY (original_event) REFERENCES event (event_id)
	ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE facility_status_event ADD CONSTRAINT FK_fac_status_event_event 
	FOREIGN KEY (event) REFERENCES event (event_id)
	ON DELETE RESTRICT ON UPDATE CASCADE;

ALTER TABLE facility_status_event ADD CONSTRAINT FK_fac_status_event_facility_status 
...
ALTER TABLE note ADD CONSTRAINT FK_note_logbook 
