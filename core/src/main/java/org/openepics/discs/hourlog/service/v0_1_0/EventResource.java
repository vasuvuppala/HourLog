...
public class EventResource {
    ...
    public List<EventRep> getEvents(@QueryParam("facility") String facilityName,
        ...
        List<Event> events;
        List<EventRep> resources = new ArrayList<>();
        ...
        events = eventEJB.findEvents(facility, start, size, from.getDate(), to.getDate());
        logger.log(Level.FINE, "EventResource#getEvents {0}", events.size());
        EventRep eventRep;
        LogEntry logEntry;
        String logText = null;
        for (Event event : events) {
            if (event.getNote() != null) {
                logEntry = logbookEJB.getEntry(event.getNote());
                if (logEntry != null) {
                    logText = logEntry.getLogText();
                } else {
                    logger.log(Level.WARNING, "EventResource#getEvents: No log entry found for id {0}", event.getNote());
                }
            } else {
                logger.log(Level.WARNING, "EventResource#getEvents: Even with an null note {0}", event.getEventId());
            }
            eventRep = EventRep.newInstance(event, logText);
            resources.add(eventRep);
        }
        return resources;
    }
}
